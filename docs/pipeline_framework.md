...
DingTalk A1 / 钉钉 AI 听记的全天原始音频，经 VAD 预过滤、智能切分、ASR+SD、声纹比对后，由 Qwen3-Max 262K 大模型生成结构化日报。

### 三大阶段

整个系统由三大阶段串联而成，前两个阶段负责"从音频到文本"，第三个阶段负责"从文本到日报"：

```
═══════════════════════════════════════════════════════════════════════════════════
  阶段 A: 音频预处理                阶段 B: ASR + 说话人识别           阶段 C: 智能摘要
  (本文档 Stage 0-1)               (本文档 Stage 2-3)               (architecture.md)
═══════════════════════════════════════════════════════════════════════════════════

  ┌──────────┐   ┌──────────┐     ┌──────────┐   ┌──────────┐     ┌──────────────┐
  │ Stage 0  │   │ Stage 1  │     │ Stage 2  │   │ Stage 3  │     │   Stage 4    │
  │ VAD 预过滤│──▶│ 智能切分  │────▶│ Tingwu   │──▶│ 说话人    │────▶│  摘要总结     │
  │ (可选)    │   │ (≤6h/段) │     │ ASR+SD   │   │ 映射     │     │  Pipeline    │
  └──────────┘   └──────────┘     └──────────┘   └──────────┘     └──────────────┘
   Silero VAD     基于 VAD 边界     通义听悟并行     已注册→声纹比对    Qwen3-Max 262K
   阈值 0.35      + 静默段切分      提交+轮询       未注册→保留编号    两级弹性架构
   仅标记不裁剪    跳过>30min静默    overlap 去重     (V2: 跨段聚类)    → 结构化日报
```
...
| 阶段 | Stage | 职责 | 详细设计文档 |
|:-----|:------|:-----|:------------|
| **A. 音频预处理** | Stage 0: VAD 预过滤 | Silero VAD 标记有效语音段，供切分参考 | 本文档 §2 |
| | Stage 1: 智能切分 | 将 >6h 音频切为 ≤6h 片段，跳过大块静默 | 本文档 §3 |
| **B. ASR + 说话人** | Stage 2: Tingwu ASR+SD | 并行调用通义听悟，获取转写+说话人分离 | 本文档 §4 |
| | Stage 3: 说话人映射 | 声纹比对 → 关键人识别，输出统一转写结果 | 本文档 §5 |
| **C. 智能摘要** | Stage 4: 摘要总结 | 关键人评分 → Qwen3-Max 生成 → 结构化日报 | [architecture.md](architecture.md) |
...
| 支持录音时长 | 3-24 小时（动态适配） |
| Tingwu 单次上限 | 6 小时 |
| VAD 处理速度 | ~2-3 分钟 / 24h 音频（CPU） |
| ASR 处理时间 | ~15-30 分钟 / 6h 片段 |
| 切分 overlap | 前后各 30 秒 |
| VAD 成本节省 | 17%-58%（按录音时长） |
| 摘要模型 | Qwen3-Max 262K tokens |
| 摘要调用次数 | 1-4 次（两级弹性） |
| 端到端日成本 | ¥1.5-3.5（VAD 开启，含 Tingwu + Qwen3） |
...
║  [Stage 3] 说话人映射                  ║
║      │ 已注册声纹 → 真实姓名            ║
║      │ 未注册 → chunk_X_speaker_Y      ║
║      │ 输出: MergedTranscript          ║
╚══════════════════════════════════════╝
    │
    ▼
╔══════════════════════════════════════╗
║  阶段 C: 智能摘要 (architecture.md)   ║
║                                      ║
║  [Stage 4] summary_bridge.py 格式转换 ║
║      │                                ║
║      ▼                                ║
║  会话检测 → 预处理 → 关键人匹配         ║
║      │                                ║
║      ▼                                ║
║  重要性评分（MVP: Base + LLM×10）      ║
║      │                                ║
║      ▼                                ║
║  Qwen3-Max 摘要生成（两级弹性）         ║
║      │ 短日(<80K): 单次调用             ║
...
> 以下为各 Stage 的详细设计。阶段 C（Stage 4）的完整设计见 [architecture.md](architecture.md)。
